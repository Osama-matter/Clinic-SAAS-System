...
const ClinicsManagement = lazy(() => import("../components/ClinicsManagement"));
const SubscriptionStatusCard = lazy(() => import("../components/SubscriptionStatusCard"));
const DoctorDashboardPanel = lazy(() => import("../components/doctor/DoctorDashboardPanel"));

const DeferredSectionFallback = () => (
  <div className="card-premium p-6 sm:p-8 animate-pulse">
    <div className="h-5 w-32 rounded-full bg-slate-100 mb-5" />
...
const DashboardPage = () => {
  ...
  const subscriptionSection = useDeferredSection();
  const clinicsSection = useDeferredSection();
  ...
  return (
    <Layout title={t("dashboard")}>
      <div
        className="max-w-[1400px] mx-auto space-y-4 md:space-y-6 lg:space-y-10 dark:text-slate-100 px-1 sm:px-0"
        dir={isRtl ? "rtl" : "ltr"}
      >
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 md:py-32">
            ...
            <p className="text-slate-400 font-black uppercase tracking-[0.2em] text-[9px] md:text-[10px] mt-6">
              {t("loadingDashboard")}
            </p>
          </div>
        ) : (
          <div className="space-y-6 md:space-y-10 animate-fade-in">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 lg:gap-6">
              {statCards.map((card, i) => (
                <div key={i} className="card-premium group p-4 sm:p-6 lg:p-8 flex items-center gap-4 sm:gap-6">
                  <div
                    className={`w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl flex items-center justify-center shrink-0 border transition-all scale-100 group-hover:scale-110 group-active:scale-95 group-hover:shadow-lg ${card.color}`}
                  >
                    {React.cloneElement(card.icon, { className: "w-5 h-5 sm:w-6 sm:h-6" })}
                  </div>
                  <div className="min-w-0">
                    <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-slate-400 mb-0.5 truncate">
                      {card.label}
                    </p>
                    <p className="text-xl sm:text-2xl lg:text-3xl font-black text-on-surface font-headline leading-none">
                      {card.value}
                    </p>
                  </div>
                </div>
            ...
            {isDoctor && (
              <Suspense fallback={<DeferredSectionFallback />}>
                <DoctorDashboardPanel lang={lang} />
              </Suspense>
            )}

            {isAdmin && (
              <div ref={subscriptionSection.ref} className="pt-2">
                {subscriptionSection.isVisible ? (
                  <Suspense fallback={<DeferredSectionFallback />}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 lg:gap-8">
                <div className="card-premium relative overflow-hidden group p-5 sm:p-8 lg:p-10 flex flex-col h-full">
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    <p className="text-xs md:text-sm text-slate-500 font-medium mb-8 md:mb-10 flex-1 leading-relaxed max-w-md">
                      {isAr
                        ? "قم بمراجعة وإدارة مواعيد العيادة وجداول الأطباء بكفاءة عالية."
                        : "View and manage clinical appointments and doctor schedules with high efficiency."}
                    </p>
                ...
                <div className="card-premium bg-gradient-to-br from-emerald-50/50 to-transparent border-emerald-100 relative overflow-hidden group p-5 sm:p-8 lg:p-10 flex flex-col h-full">
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    <p className="text-xs md:text-sm text-slate-500 font-medium mb-8 md:mb-10 flex-1 leading-relaxed max-w-md">
                      {isAr
                        ? "تتبع أداء العيادة، نمو المرضى، والإحصائيات الحيوية لاتخاذ قرارات مدروسة."
                        : "Track clinic performance, patient growth, and vital stats to make informed decisions."}
                    </p>
                    <button
            ...
            {isSuperAdmin && (
              <div ref={clinicsSection.ref} className="animate-fade-in pt-8 border-t border-outline/50">
                {clinicsSection.isVisible ? (
                  <Suspense fallback={<DeferredSectionFallback />}>
                    <ClinicsManagement />
                  </Suspense>
                ) : (
