...
const MyBookingsPage = () => {
  ...
  const loadBookings = async () => {
    ...
    try {
      ...
      toast.error(isAr ? "فشل تحميل المواعيد." : "Failed to load appointments.");
    }
      ...
      <div className="max-w-6xl mx-auto space-y-12 pb-24" dir={isRtl ? "rtl" : "ltr"}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {[
            ...
            <div key={label} onClick={() => setFilter(status)}
              className={`card-premium p-4 sm:p-6 cursor-pointer group transition-all border-2 ${
                filter === status ? "border-primary shadow-xl shadow-primary/10 scale-105" : "border-transparent bg-white hover:border-slate-200"
              }`}>
              <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2 sm:mb-3 group-hover:text-primary transition-colors">{label}</p>
              <p className={`text-2xl sm:text-4xl font-black font-headline ${filter === status ? 'text-slate-900' : 'text-slate-300'}`}>{value}</p>
            </div>
          ))}
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">{t('syncingRecords') || (isAr ? "جاري مزامنة السجلات..." : "Syncing records...")}</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="card-premium p-20 text-center animate-fade-in bg-white border border-slate-200">
            ...
            <p className="text-slate-500 font-medium mb-12 max-w-sm mx-auto leading-relaxed">{t('noAppointmentsFoundSubtitle')}</p>
              ...
              <div key={b.id}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-y-3 gap-x-8 text-sm font-medium">
                    <div className="flex items-center gap-3 text-slate-600">
                      ...
                      {b.slotDateTime ? new Date(b.slotDateTime).toLocaleDateString(lang, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : "—"}
                    </div>
                    <div className="flex items-center gap-3 text-slate-600">
                      <Clock className="w-4 h-4 text-primary opacity-40" />
                      {b.slotDateTime ? new Date(b.slotDateTime).toLocaleTimeString(lang, { hour: '2-digit', minute: '2-digit' }) : "—"}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3 flex-shrink-0 self-end md:self-center">
                  {b.status !== "Cancelled" && b.status !== "Completed" && b.status !== "NoShow" && (
                    <>
                      <button onClick={() => { setModal(b); setNewStatus(b.status); }}
          ...
          <form onSubmit={handleUpdate} className="space-y-8" dir={isRtl ? "rtl" : "ltr"}>
            <div className="p-6 rounded-3xl bg-slate-50 border border-slate-200 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">Ref: {statusModal.bookingReference}</p>
              </div>
            </div>
            ...
            <div className="flex gap-4 pt-4">
              ...
              <button type="submit" disabled={updating}
                ...
                {updating ? <><Loader2 className="w-4 h-4 animate-spin"/> {t('updatingStatus')}</> : (
                  <>
                    {t('updateStatus')}
