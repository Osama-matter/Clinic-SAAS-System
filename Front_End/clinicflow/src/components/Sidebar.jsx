...
const SectionLabel = ({ label }) => (
  <div className="px-4 pt-6 pb-2 first:pt-2">
    <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-on-surface-variant/40">{label}</p>
  </div>
);
...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const getSections = () => {
    if (isAdmin) {
      return [
        { label: lang === "ar" ? "نظرة عامة" : "Overview", items: [
        ...
        { label: lang === "ar" ? "الإدارة" : "Management", items: [
        ...
        { label: lang === "ar" ? "النظام" : "System", items: [
    ...
    if (isDoctor) {
      return [
        ...
        { label: lang === "ar" ? "الدليل" : "Directory", items: [
          { label: lang === "ar" ? "المرضى" : "Patients", icon: <Users className="w-[18px] h-[18px]" />, path: "/patients" },
          { label: t('doctors'), icon: <UserRound className="w-[18px] h-[18px]" />, path: "/doctors" },
        ]},
      ];
    ...
    return [
      ...
      { label: lang === "ar" ? "المواعيد" : "Appointments", items: [
  ...
  const getRoleBadge = () => {
    if (isAdmin) return { label: lang === "ar" ? "مدير" : "Admin", color: "bg-purple-500/10 text-purple-400 border-purple-500/20" };
    if (isDoctor) return { label: lang === "ar" ? "طبيب" : "Doctor", color: "bg-blue-500/10 text-blue-400 border-blue-500/20" };
    return { label: lang === "ar" ? "مريض" : "Patient", color: "bg-green-500/10 text-green-400 border-green-500/20" };
  };

  const roleBadge = getRoleBadge();

  return (
    <aside className={`fixed lg:sticky top-0 ${isRtl ? 'right-0' : 'left-0'} z-[70] lg:z-20 w-[280px] h-full bg-surface dark:bg-slate-900 border-${isRtl ? 'l' : 'r'} border-outline dark:border-white/5 flex flex-col shrink-0 transition-transform duration-300 ease-in-out lg:translate-x-0 ${
      ...
      <div className="px-8 h-[80px] flex items-center justify-between shrink-0 border-b border-outline bg-surface-alt">
        <NavLink to="/dashboard" className="flex items-center gap-3 group" onClick={onClose}>
          ...
          <span className="font-headline font-black text-2xl tracking-tighter text-slate-900 dark:text-white leading-none">
            Clinic<span className="text-primary italic">Flow</span>
          </span>
        </NavLink>
